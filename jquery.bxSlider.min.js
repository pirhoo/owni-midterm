...
checkEndControls();if(options.moveSlideQty>1&&!options.infiniteLoop){number=Math.ceil(number/options.moveSlideQty);}
...
function showPager(type){var pagerQty=$children.length;if(options.moveSlideQty>1&&!options.infiniteLoop){if($children.length%options.moveSlideQty!=0){pagerQty=Math.ceil($children.length/options.moveSlideQty);}else{pagerQty=$children.length/options.moveSlideQty;}}
...
$pager.children().click(function(){if(options.pagerType=='full'){var slideIndex=$pager.children().index(this);if(options.moveSlideQty>1&&!options.infiniteLoop){slideIndex*=options.moveSlideQty;}
